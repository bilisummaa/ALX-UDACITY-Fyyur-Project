...
#app.config['SQLALCHEMY_DATABASE_URI'] = 'postgresql://ararsal:aararsal@localhost:5432/fyyur'
...
class Shows(db.Model):
  _tablename__ = 'shows'
  db.relationship("Venue")
  id = db.Column(db.Integer, primary_key=True)
  day_dates = db.Column(db.String(120), nullable=False)
...
def venue():
  ...
  for venue in data:
    ...
    if len(venue_details) == 0:
      ...
      venue_details.append(venue_details_list)
    else:
      for i, area in enumerate(venue_details):
      ...
      if pos_venue < 0:
        ...
        venue_details.append(venue_details_list)
        pos_venue = len(venue_details) - 1
      else:
        venue_details_list = venue_details[pos_venue]
    v = {
        "id": venue.id,
        "name": venue.name,
        "num_upcoming_shows": 4
      }
    venue_details_list['venues'].append(v)
    venue_details[pos_venue] = venue_details_list

  return render_template('pages/venues.html', venue_details=venue_details)

@app.route('/venue/search', methods=['POST'])
def search_venue():
  search_term = request.form.get('search_term')
  search = "%{}%".format(search_term)
  data = Venue.query.filter(Venue.name.ilike(search)).order_by('name').all()
  items = []
  for row in data:
    aux = {
      "id": row.id,
...
def show_venue(venue_id):
  ...
  for show in data.shows:
    if show.day_dates > datetime.now():
...
def create_venue_submission():
  ...
  if error:
    abort(500)
    body['success'] = False
    body['msg'] = 'Venue cannot be created. Something went wrong. '
  else:
    body['msg'] = 'Venue is created  sucessfully'
    body['success'] = True
...
def search_artist(): 
  ...
  data = Artist.query.filter(Artist.name.ilike(search_term)).order_by('name').all()
  ...
  for row in data:
    ...
    search_artist_result.append(aux)
  response={
...
def edit_artist_submission(artist_id):
  ...
  if error:
    abort(500)
    body['success'] = False
    body['msg'] = 'Error: Artist cannot be updated. '
  else:
    body['msg'] = 'Show is Edited sucessfully'
    body['success'] = True
...
def edit_venue_submission(venue_id):
  ...
  if error:
    abort(500)
    body['success'] = False
    body['msg'] = 'Error: Venue cannot be updated.'
  else:
    body['msg'] = 'Venue is edited sucessfully'
    body['success'] = True
...
def create_artist_submission():
  ...
  if error:
    abort(500)
    body['success'] = False
    body['msg'] = 'Error: Artist cannot be created '
  else:
    body['msg'] = 'Artist is created  sucessfully'
    body['success'] = True
...
@app.route('/shows')
def shows():
  rows = db.session.query(Shows, Artist, Venue).join(Artist).join(Venue).filter(Shows.day_dates > datetime.now()).order_by('day_dates').all()
  data = []
  for row in rows:
    item = {
      ...
      'start_time': row.Shows.day_dates.strftime('%Y-%m-%d %H:%I')
    }
    data.append(item)
...
def create_show_submission():
  ...
  try:
    ...
    show = Shows(artist_id=artist_id, venue_id=venue_id, day_dates=start_time)
    db.session.add(show)
    db.session.commit()
  except:
    db.session.rollback()
  ...
  if error:
    abort(500)
    body['success'] = False
    body['msg'] = 'Error: Show cannot be cteated. '
  else:
    body['msg'] = 'Show is created sucessfully'
    body['success'] = True
